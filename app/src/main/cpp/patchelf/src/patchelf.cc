...
splitColonDelimitedString(std::string_view s) {
...
static bool hasAllowedPrefix(const std::string &s,
                             const std::vector<std::string> &allowedPrefixes) {
  return std::any_of(
      allowedPrefixes.begin(), allowedPrefixes.end(),
...
[[nodiscard]] static ElfType getElfType(const FileContents &fileContents) {
  ...
  // FIXME: endianness
  return ElfType{
      is32Bit, is32Bit ? (reinterpret_cast<Elf32_Ehdr *>(contents))->e_machine
                       : (reinterpret_cast<Elf64_Ehdr *>(contents))->e_machine};
}

static void checkPointer(const FileContents &contents, const void *p,
                         size_t size) {
  if (p < contents->data() || size > contents->size() ||
      p > contents->data() + contents->size() - size)
    error("data region extends past file end");
}

static void checkOffset(const FileContents &contents, size_t offset,
                        size_t size) {
  size_t end;

  if (offset > contents->size() || size > contents->size() ||
      __builtin_add_overflow(offset, size, &end) || end > contents->size())
    error("data offset extends past file end");
}

static std::string extractString(const FileContents &contents, size_t offset,
                                 size_t size) {
...
ElfFile<ElfFileParamNames>::ElfFile(FileContents fContents)
  ...
  size_t shstrtabptr;
  if (__builtin_add_overflow(reinterpret_cast<size_t>(fileContents->data()),
                             rdi(shdrs[shstrtabIndex].sh_offset), &shstrtabptr))
...
unsigned int ElfFile<ElfFileParamNames>::getPageSize() const noexcept {
  ...
  case EM_SPARC: // This should be sparc 32-bit. According to the linux
                 // kernel 4KB should be also fine, but it seems that solaris is
                 // doing 8KB
...
template <ElfFileParams> void ElfFile<ElfFileParamNames>::sortShdrs() {
  ...
  for (unsigned int i = 1; i < rdi(hdr()->e_shnum); ++i)
    if (rdi(shdrs.at(i).sh_link) != 0)
      linkage[getSectionName(shdrs.at(i))] =
          getSectionName(shdrs.at(rdi(shdrs.at(i).sh_link)));

  /* Idem for sh_info on certain sections. */
  std::map<SectionName, SectionName> info;
  for (unsigned int i = 1; i < rdi(hdr()->e_shnum); ++i)
    if (rdi(shdrs.at(i).sh_info) != 0 && (rdi(shdrs.at(i).sh_type) == SHT_REL ||
                                          rdi(shdrs.at(i).sh_type) == SHT_RELA))
      info[getSectionName(shdrs.at(i))] =
          getSectionName(shdrs.at(rdi(shdrs.at(i).sh_info)));
  ...
  for (unsigned int i = 1; i < rdi(hdr()->e_shnum); ++i)
    if (rdi(shdrs[i].sh_link) != 0)
      wri(shdrs[i].sh_link, getSectionIndex(linkage[getSectionName(shdrs[i])]));
  ...
  /* And the .shstrtab index. Note: the match here is done by checking the
   * offset as searching by name can yield incorrect results in case there are
   * multiple sections with the same name as the one initially pointed by
   * hdr()->e_shstrndx */
  for (unsigned int i = 1; i < rdi(hdr()->e_shnum); ++i) {
...
static void writeFile(const std::string &fileName,
  ...
  int fd =
      open(fileName.c_str(), O_CREAT | O_TRUNC | O_WRONLY | O_BINARY, 0777);
  if (fd == -1)
    error("open");
...
void ElfFile<ElfFileParamNames>::shiftFile(unsigned int extraPages,
  ...
  /* Move the entire contents of the file after 'startOffset' by 'extraPages'
   * pages further. */
  unsigned int shift = extraPages * getPageSize();
  fileContents->resize(oldSize + shift, 0);
  memmove(fileContents->data() + startOffset + shift,
          fileContents->data() + startOffset, oldSize - startOffset);
  ...
  for (int i = 0; i < rdi(hdr()->e_phnum); ++i) {
    ...
    if (p_start <= startOffset &&
        p_start + rdi(phdrs.at(i).p_filesz) > startOffset &&
        rdi(phdrs.at(i).p_type) == PT_LOAD) {
      assert(splitIndex == -1);

      splitIndex = i;
      splitShift = startOffset - p_start;

      /* This is the load segment we're currently extending within, so we split
       * it. */
    ...
    if (p_start >= startOffset) {
      ...
      if (rdi(phdrs.at(i).p_align) != 0 &&
          (rdi(phdrs.at(i).p_vaddr) - rdi(phdrs.at(i).p_offset)) %
                  rdi(phdrs.at(i).p_align) !=
              0) {
...
template <ElfFileParams>
const Elf_Shdr &ElfFile<ElfFileParamNames>::findSectionHeader(
...
ElfFile<ElfFileParamNames>::tryFindSectionHeader(
    const SectionName &sectionName) const {
...
span<T> ElfFile<ElfFileParamNames>::getSectionSpan(const Elf_Shdr &shdr) const {
  return span((T *)(fileContents->data() + rdi(shdr.sh_offset)),
              rdi(shdr.sh_size) / sizeof(T));
}

template <ElfFileParams>
template <class T>
span<T>
ElfFile<ElfFileParamNames>::getSectionSpan(const SectionName &sectionName) {
...
template <class T>
span<T>
ElfFile<ElfFileParamNames>::tryGetSectionSpan(const SectionName &sectionName) {
  auto shdrOpt = tryFindSectionHeader(sectionName);
  return shdrOpt ? getSectionSpan<T>(*shdrOpt) : span<T>();
}

template <ElfFileParams>
unsigned int ElfFile<ElfFileParamNames>::getSectionIndex(
    const SectionName &sectionName) const {
  for (unsigned int i = 1; i < rdi(hdr()->e_shnum); ++i)
    if (getSectionName(shdrs.at(i)) == sectionName)
      return i;
  return 0;
}
...
void ElfFile<ElfFileParamNames>::writeReplacedSections(Elf_Off &curOff,
  ...
  for (auto &i : replacedSections) {
    ...
    if (rdi(shdr.sh_type) != SHT_NOBITS)
      memset(fileContents->data() + rdi(shdr.sh_offset), 'Z',
             rdi(shdr.sh_size));
  }
  ...
  for (auto &shdr : shdrs) {
    ...
    memcpy(fileContents->data() + curOff, i->second.c_str(), i->second.size());
...
void ElfFile<ElfFileParamNames>::rewriteSectionsLibrary() {
  ...
  unsigned int num_notes =
      std::count_if(shdrs.begin(), shdrs.end(), [this](Elf_Shdr shdr) {
        return rdi(shdr.sh_type) == SHT_NOTE;
      });
  ...
  unsigned int i = 1;
  Elf_Addr pht_size =
      sizeof(Elf_Ehdr) + (phdrs.size() + num_notes + 1) * sizeof(Elf_Phdr);
  ...
  off_t headerTableSpace =
      roundUp(rdi(hdr()->e_shnum) * rdi(hdr()->e_shentsize), sectionAlignment);
  ...
  if (!phdrs.empty() && rdi(lastSeg.p_type) == PT_LOAD &&
      rdi(lastSeg.p_flags) == (PF_R | PF_W) &&
      rdi(lastSeg.p_align) == alignStartPage) {
    auto segEnd =
        roundUp(rdi(lastSeg.p_offset) + rdi(lastSeg.p_memsz), getPageSize());
...
void ElfFile<ElfFileParamNames>::rewriteSectionsExecutable() {
  ...
  if (neededSpace > startOffset) {
    ...
    unsigned int neededPages =
        1 + roundUp(extraSpace, getPageSize()) / getPageSize();
    ...
    Elf_Off rewrittenSectionsOffset =
        sizeof(Elf_Ehdr) + phdrs.size() * sizeof(Elf_Phdr);
    for (auto &phdr : phdrs)
...
void ElfFile<ElfFileParamNames>::normalizeNoteSegments() {
  ...
  bool replaced_note =
      std::any_of(replacedSections.begin(), replacedSections.end(),
                  [this](std::pair<const std::string, std::string> &i) {
                    return rdi(findSectionHeader(i.first).sh_type) == SHT_NOTE;
                  });
  if (!replaced_note)
    return;
  ...
  for (auto &phdr : phdrs) {
    ...
    /* Binaries produced by older patchelf versions may contain empty PT_NOTE
       segments. For backwards compatibility, if we find one we should ignore
       it. */
    bool empty = std::none_of(shdrs.begin(), shdrs.end(), [&](auto &shdr) {
      return rdi(shdr.sh_offset) >= start_off && rdi(shdr.sh_offset) < end_off;
    });
    if (empty)
      continue;
    ...
    while (curr_off < end_off) {
      ...
      for (const auto &shdr : shdrs) {
        if (rdi(shdr.sh_type) != SHT_NOTE)
          continue;
        if (rdi(shdr.sh_offset) != roundUp(curr_off, rdi(shdr.sh_addralign)))
          continue;
      ...
      if (size == 0)
        error("cannot normalize PT_NOTE segment: non-contiguous SHT_NOTE "
              "sections");
      if (curr_off + size > end_off)
        error("cannot normalize PT_NOTE segment: partially mapped SHT_NOTE "
              "section");
...
void ElfFile<ElfFileParamNames>::rewriteSections(bool force) {
  ...
  if (!force && replacedSections.empty())
    return;
...
void ElfFile<ElfFileParamNames>::rewriteHeaders(Elf_Addr phdrAddress) {
  ...
  if (shdrDynamic) {
    auto dyn_table =
        (Elf_Dyn *)(fileContents->data() + rdi((*shdrDynamic).get().sh_offset));
    unsigned int d_tag;
    for (auto dyn = dyn_table; (d_tag = rdi(dyn->d_tag)) != DT_NULL; dyn++)
      if (d_tag == DT_STRTAB)
        ...
        if (!shdr)
          shdr = tryFindSectionHeader(".rela.plt");
        /* 64-bit Linux, x86-64 */
        if (!shdr)
          shdr = tryFindSectionHeader(
              ".rela.IA_64.pltoff"); /* 64-bit Linux, IA-64 */
        if (!shdr)
        ...
        /* no idea if this makes sense, but it was needed for some
           program */
        if (!shdr)
          shdr = tryFindSectionHeader(".rel.got");
        ...
        if (shdr) {
          /*
           * "When correct, (DT_MIPS_RLD_MAP_REL + tag offset + executable base
           * address) equals DT_MIPS_RLD_MAP"
           * -- https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=820334#5
           *
           * Equivalently,
           *
           *   DT_MIPS_RLD_MAP_REL + tag offset + executable base address ==
           * DT_MIPS_RLD_MAP DT_MIPS_RLD_MAP_REL              + executable base
           * address == DT_MIPS_RLD_MAP - tag_offset DT_MIPS_RLD_MAP_REL ==
          ...
          dyn->d_un.d_ptr =
              rld_map_addr - dyn_offset - (*shdrDynamic).get().sh_addr;
        } else {
          ...
          fprintf(stderr, "warning: DT_MIPS_RLD_MAP_REL entry is present, but "
                          ".rld_map section is not\n");
          dyn->d_un.d_ptr = 0;
        }
      }
  }
  ...
  for (unsigned int i = 1; i < rdi(hdr()->e_shnum); ++i) {
    ...
    if (rdi(shdr.sh_type) != SHT_SYMTAB && rdi(shdr.sh_type) != SHT_DYNSYM)
      continue;
    debug("rewriting symbol table section %d\n", i);
    for (size_t entry = 0; (entry + 1) * sizeof(Elf_Sym) <= rdi(shdr.sh_size);
         entry++) {
      auto sym = (Elf_Sym *)(fileContents->data() + rdi(shdr.sh_offset) +
...
template <ElfFileParams>
void ElfFile<ElfFileParamNames>::modifyOsAbi(osAbiMode op,
                                             const std::string &newOsAbi) {
...
void ElfFile<ElfFileParamNames>::modifySoname(sonameMode op,
  ...
  if (soname && soname == newSoname) {
    debug(
        "current and proposed new SONAMEs are equal keeping DT_SONAME entry\n");
    return;
  }
  ...
  std::string &newDynStr =
      replaceSection(".dynstr", rdi(shdrDynStr.sh_size) + newSoname.size() + 1);
  ...
  if (dynSoname) {
    ...
    std::string &newDynamic =
        replaceSection(".dynamic", rdi(shdrDynamic.sh_size) + sizeof(Elf_Dyn));

    unsigned int idx = 0;
    for (; rdi(reinterpret_cast<const Elf_Dyn *>(newDynamic.c_str())[idx]
                   .d_tag) != DT_NULL;
         idx++)
      ;
...
template <ElfFileParams>
void ElfFile<ElfFileParamNames>::setInterpreter(
    const std::string &newInterpreter) {
...
/* For each directory in the RPATH, check if it contains any
   needed library. */
template <ElfFileParams>
std::string ElfFile<ElfFileParamNames>::shrinkRPath(
    char *rpath, std::vector<std::string> &neededLibs,
  ...
  for (auto &dirName : splitColonDelimitedString(rpath)) {
    ...
    if (!allowedRpathPrefixes.empty() &&
        !hasAllowedPrefix(dirName, allowedRpathPrefixes)) {
      debug(
          "removing directory '%s' from RPATH because of non-allowed prefix\n",
          dirName.c_str());
      continue;
    }
...
template <ElfFileParams>
void ElfFile<ElfFileParamNames>::modifyRPath(
    RPathOp op, const std::vector<std::string> &allowedRpathPrefixes,
    std::string newRPath) {
  auto shdrDynamic = findSectionHeader(".dynamic");
  ...
  case rpSet: {
    break;
  } /* new rpath was provied as input to this function */
  }

  if (!forceRPath && dynRPath &&
      !dynRunPath) { /* convert DT_RPATH to DT_RUNPATH */
...
template <ElfFileParams>
void ElfFile<ElfFileParamNames>::removeNeeded(
...
template <ElfFileParams>
void ElfFile<ElfFileParamNames>::replaceNeeded(
    const std::map<std::string, std::string> &libs) {
  if (libs.empty())
    return;
  ...
  for (; rdi(dyn->d_tag) != DT_NULL; dyn++) {
    if (rdi(dyn->d_tag) == DT_NEEDED) {
      ...
      if (i != libs.end() && name != i->second) {
        ...
        std::string &newDynStr =
            replaceSection(".dynstr", strOffset + replacement.size() + 1);
  ...
  if (verNeedNum) {
    ...
    char *verStrTab =
        (char *)fileContents->data() + rdi(shdrVersionRStrings.sh_offset);
    ...
    // It may be that it is .dynstr again, in which case we must take the
    // already added bytes into account.
    ...
    auto need =
        (Elf_Verneed *)(fileContents->data() + rdi(shdrVersionR.sh_offset));
    while (verNeedNum > 0) {
      ...
      if (i != libs.end() && file != i->second) {
        ...
        if (a != addedStrings.end()) {
          ...
          Elf_Off strOffset =
              rdi(shdrVersionRStrings.sh_size) + verStrAddedBytes;
          std::string &newVerDynStr = replaceSection(
              versionRStringsSName, strOffset + replacement.size() + 1);
...
void ElfFile<ElfFileParamNames>::addNeeded(const std::set<std::string> &libs) {
  ...
  unsigned int idx = 0;
  for (;
       rdi(reinterpret_cast<const Elf_Dyn *>(newDynamic.c_str())[idx].d_tag) !=
       DT_NULL;
       idx++)
    ;
  debug("DT_NULL index is %d\n", idx);
...
void ElfFile<ElfFileParamNames>::printNeededLibs() const {
  ...
  const char *strTab =
      (const char *)fileContents->data() + rdi(shdrDynStr.sh_offset);

  const Elf_Dyn *dyn =
      (const Elf_Dyn *)(fileContents->data() + rdi(shdrDynamic.sh_offset));
...
template <ElfFileParams> void ElfFile<ElfFileParamNames>::noDefaultLib() {
  auto shdrDynamic = findSectionHeader(".dynamic");
...
template <ElfFileParams> void ElfFile<ElfFileParamNames>::addDebugTag() {
  auto shdrDynamic = findSectionHeader(".dynamic");
...
template <ElfFileParams>
auto ElfFile<ElfFileParamNames>::parseGnuHashTable(span<char> sectionData)
    -> GnuHashTable {
  auto hdr = (typename GnuHashTable::Header *)sectionData.begin();
  auto bloomFilters =
      span((typename GnuHashTable::BloomWord *)(hdr + 1), rdi(hdr->maskwords));
  auto buckets = span((uint32_t *)bloomFilters.end(), rdi(hdr->numBuckets));
  auto table =
      span(buckets.end(), ((uint32_t *)sectionData.end()) - buckets.end());
  return GnuHashTable{*hdr, bloomFilters, buckets, table};
}

template <ElfFileParams>
void ElfFile<ElfFileParamNames>::rebuildGnuHashTable(span<char> strTab,
                                                     span<Elf_Sym> dynsyms) {
  ...
  // Sort the entries based on the buckets. This is a requirement for gnu hash
  // table to work
  std::sort(entries.begin(), entries.end(),
            [&](auto &l, auto &r) { return l.bucketIdx < r.bucketIdx; });
  ...
  auto remapSymbolId = [&old2new, firstSymIdx](auto &oldSymIdx) {
    return oldSymIdx >= firstSymIdx
               ? old2new[oldSymIdx - firstSymIdx] + firstSymIdx
               : oldSymIdx;
  };
  ...
  for (size_t i = 0; i < entries.size(); ++i) {
    auto &n = entries[i];
    bool isLast =
        (i == entries.size() - 1) || (n.bucketIdx != entries[i + 1].bucketIdx);
...
template <ElfFileParams>
auto ElfFile<ElfFileParamNames>::parseHashTable(span<char> sectionData)
    -> HashTable {
...
template <ElfFileParams>
void ElfFile<ElfFileParamNames>::rebuildHashTable(span<char> strTab,
                                                  span<Elf_Sym> dynsyms) {
...
template <ElfFileParams>
void ElfFile<ElfFileParamNames>::renameDynamicSymbols(
    const std::unordered_map<std::string_view, std::string> &remap) {
  ...
  for (auto &dynsym : dynsyms) {
    ...
    if (it != remap.end()) {
      ...
      debug("renaming dynamic symbol %s to %s\n", name.data(),
            it->second.c_str());
      extraStrings.insert(extraStrings.end(), newName.begin(),
                          newName.end() + 1);
      changed = true;
    } else {
  ...
  if (!extraStrings.empty()) {
    ...
    std::copy(extraStrings.begin(), extraStrings.end(),
              &newStrTabSpan[strTab.size()]);
...
template <ElfFileParams>
void ElfFile<ElfFileParamNames>::clearSymbolVersions(
  ...
  auto versyms =
      (Elf_Versym *)(fileContents->data() + rdi(shdrVersym.sh_offset));
...
void ElfFile<ElfFileParamNames>::modifyExecstack(ExecstackMode op) {
  if (op == ExecstackMode::clear || op == ExecstackMode::set) {
    ...
    for (size_t i = 0; i < phdrs.size(); i++) {
      ...
      if (op == ExecstackMode::clear && (rdi(header.p_flags) & PF_X) == PF_X) {
        ...
        *((Elf_Phdr *)(fileContents->data() + rdi(hdr()->e_phoff)) + i) =
            header;
        changed = true;
      } else if (op == ExecstackMode::set &&
                 (rdi(header.p_flags) & PF_X) != PF_X) {
    ...
    if (nullhdr != (size_t)-1) {
      ...
      *((Elf_Phdr *)(fileContents->data() + rdi(hdr()->e_phoff)) + nullhdr) =
          header;
      changed = true;
      return;
    }
...
void ElfFile<ElfFileParamNames>::forAllStringReferences(
    const Elf_Shdr &strTabHdr, StrIdxCallback &&fn) {
  ...
  if (auto verdHdr = tryFindSectionHeader(".gnu.version_d")) {
    if (&shdrs.at(rdi(verdHdr->get().sh_link)) == &strTabHdr)
      forAll_ElfVer(
          getSectionSpan<Elf_Verdef>(*verdHdr), [](auto & /*vd*/) {},
  ...
  if (auto vernHdr = tryFindSectionHeader(".gnu.version_r")) {
    if (&shdrs.at(rdi(vernHdr->get().sh_link)) == &strTabHdr)
      forAll_ElfVer(
          getSectionSpan<Elf_Verneed>(*vernHdr),
  ...
  for (const auto &fileName : fileNames) {
    ...
    if (getElfType(fileContents).is32Bit)
      patchElf2(ElfFile<Elf32_Ehdr, Elf32_Phdr, Elf32_Shdr, Elf32_Addr,
                        Elf32_Off, Elf32_Dyn, Elf32_Sym, Elf32_Versym,
                        Elf32_Verdef, Elf32_Verdaux, Elf32_Verneed,
                        Elf32_Vernaux, Elf32_Rel, Elf32_Rela, 32>(fileContents),
                fileContents, outputFileName2);
    else
      patchElf2(ElfFile<Elf64_Ehdr, Elf64_Phdr, Elf64_Shdr, Elf64_Addr,
                        Elf64_Off, Elf64_Dyn, Elf64_Sym, Elf64_Versym,
                        Elf64_Verdef, Elf64_Verdaux, Elf64_Verneed,
                        Elf64_Vernaux, Elf64_Rel, Elf64_Rela, 64>(fileContents),
                fileContents, outputFileName2);
  }
}
...
static int mainWrapped(int argc, char **argv) {
  ...
  for (i = 1; i < argc; ++i) {
    ...
    if (arg == "--set-interpreter" || arg == "--interpreter") {
      ...
      if (forcedPageSize <= 0)
        error("invalid argument to --page-size");
      ...
      if (!infile)
        error(fmt("Cannot open map file ", fname));
      ...
      while (std::getline(infile, line)) {
        ...
        if (!(iss >> from))
          break;
        if (!(iss >> to))
          error(fmt(fname, ":", lineCount,
                    ": Map file line is missing the second element"));
        if (symbolsToRenameKeys.count(from))
          error(fmt(fname, ":", lineCount, ": Name '", from,
                    "' appears twice in the map file"));
        if (from.find('@') != std::string_view::npos ||
            to.find('@') != std::string_view::npos)
          error(fmt(fname, ":", lineCount,
                    ": Name pair contains version tag: ", from, " ", to));
        lineCount++;
